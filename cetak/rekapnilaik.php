<?php
 ...
 include '../assets/db_connect.php';
	...
	$smt=$_GET['smt'];
		...
		$namafilenya="Rekapitulasi Nilai Raport Ketrampilan ".$kelas.".pdf";
		$pdf=new exFPDF('L','mm',array(330,215));
		$pdf->AddPage(); 
		$pdf->SetFont('helvetica','',12);

		$table2=new easyTable($pdf, 2);
		$table2->easyCell('REKAPITULASI NILAI RAPORT KETRAMPILAN '.$kelas,'colspan:2;align:C;font-style:B');
		$table2->printRow();
		...
		while($sis=$qkl->fetch_assoc()){
			...
			for ($i=1; $i < 13; $i++) { 
			  $nr=$connect->query("select * from raport_k13 where id_pd='$idp' and kelas='$ab' and smt='$smt' and tapel='$tapel' and mapel='$i' and jns='k4'")->fetch_assoc();
			  if(empty($nr['nilai'])){
			...
			$nto=$connect->query("select sum(nilai) as total from raport_k13 where id_pd='$idp' and kelas='$ab' and smt='$smt' and tapel='$tapel' and jns='k4'")->fetch_assoc();
			$nrt=$connect->query("select avg(nilai) as rerata from raport_k13 where id_pd='$idp' and kelas='$ab' and smt='$smt' and tapel='$tapel' and jns='k4'")->fetch_assoc();
