<?php
 ...
 include '../assets/db_connect.php';
	...
	$smt=$_GET['smt'];
		...
		$sis=$connect->query("select * from siswa where peserta_didik_id='$idp'")->fetch_assoc();
		...
		$table2->endTable();
			...
			$table3->endTable();
			$table2=new easyTable($pdf, '{15,20,275}','border:1');
			$table2->easyCell('Nilai Raport','align:C;');
			$table2->easyCell('Predikat','align:C');
			$table2->easyCell('Deskripsi','align:C');
			$table2->printRow();
			$nr=$connect->query("select * from raport_k13 where id_pd='$idp' and kelas='$ab' and smt='$smt' and tapel='$tapel' and mapel='$mapel' and jns='k3'")->fetch_assoc();
			$table2->easyCell($nr['nilai'],'align:C;');
			$table2->easyCell($nr['predikat'],'align:C');
			$table2->easyCell($nr['deskripsi'],'align:L');
			$table2->printRow();
			$table2->endTable();
		...
		$ttd->easyCell('');
		$ttd->easyCell('');
		if($mapel==1 or $mapel==8 or $mapel==10){
			$ttd->easyCell('Guru Mata Pelajaran','align:C; valign:T');
		}else{
